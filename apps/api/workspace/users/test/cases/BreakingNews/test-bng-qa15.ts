...
export class TestBreakingNews extends TestCase {
  ...
  async login(userName,password) {
    try{
      await this.goto('https://qa15.gettr-qa.com/login?step=sea_login_with_email',{ waitUntil: "domcontentloaded" })
    }catch(err){
      this.printError(err)
      return
    }
    ...
    async postbng(userName:string,password:string){
      ...
      while(createBt && counter>0){
        ...
        const postContent = `bng test tab refresh:${this.formatDateTime(new Date())} `
