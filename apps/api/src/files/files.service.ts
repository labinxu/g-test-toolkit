...
import { checkPath, combineDtsFiles, findFilesByExtname } from 'src/common/utils'
...
export class FilesService {
  ...
  makeTypesFile(oFile: string = null) {
    ...
    methodDeclarations.push('export declare class TestCase implements ITestBase {')
    ...
    for (const sourceFile of sourceFiles) {
      ...
      for (const cls of classes) {
        ...
        if (methods.length > 0) {
          methodDeclarations.push(`// Methods from ${className}`, ...methods, '')
        }
      }
    }
