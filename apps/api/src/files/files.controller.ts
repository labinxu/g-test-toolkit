...
export class FilesController {
  ...
  async getDirTree(
    ...
    const absPath = path.resolve(path.join(process.env.WORKSPACE, dpath));
  ...
  async saveFastUserDbScript(@Body() body: { content: string }) {
    try {
      const result = await this.filesService.saveFastUserDbScript(
        body?.content ?? '',
      );
  ...
  async listVideos(@Query('depth') depth: number = 3) {
    const videosRoot = path.normalize(
      path.join(process.cwd(), 'workspace/videos'),
    );
    const baseDir = path.resolve(process.cwd());
    if (!videosRoot.startsWith(baseDir)) {
      throw new Error(
        `Access to paths outside the working directory is forbidden ${videosRoot}`,
      );
    }
  ...
  async uploadVideoFile(@UploadedFile() file: FastifyMulterFile) {
    ...
    try {
      const result = await this.filesService.saveVideoFile(
        file.buffer,
        file.originalname,
      );
      return { success: true, ...result };
